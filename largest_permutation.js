...
function largestPermutation(k, arr) {
  ...
  let highestVal = arr.length;
  let startIndex = 0;

  for (let i = 1; i <= k; i++) {
    let hightValIndex = arr.indexOf(highestVal);
    let highVal = arr[hightValIndex];
    let startVal = arr[startIndex];

    if (hightValIndex === startIndex) {
      i--;
      highestVal--;
      startIndex++;
      continue;
    ...
    arr[startIndex] = highVal;
    arr[hightValIndex] = startVal;
